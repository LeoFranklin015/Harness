...
export function Agent({
  ...
  const ink = "#e8edf5";
  const accent = stopped ? ink : "#34d399";
  ...
  return (
    ...
    >
      ...
      >
        ...
        <path d={arc(100, 26, 24, 190, 262)} style={anim("agent-wave", busy ? 1.1 : 2.2)} />
        <path
          d={arc(100, 26, 39, 196, 256)}
          style={anim("agent-wave", busy ? 1.1 : 2.2, busy ? 0.22 : 0.45)}
        />

        <g style={anim("agent-bob", 5)}>
          ...
          <g style={anim("agent-blink", 5.6)} fill={accent} stroke="none">
      ...
      {state === "live" &&
        COINS.map((delay) => (
          <g
            key={delay}
            style={{ animation: `agent-spend 4.5s linear ${delay}s infinite` }}
            stroke={accent}
            strokeWidth={4}
            strokeLinecap="round"
            fill="none"
